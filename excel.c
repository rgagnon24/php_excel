...
static zend_object_value excel_object_new_book(zend_class_entry *class_type TSRMLS_DC)
{
	...
	xlBookSetLocale(intern->book, "en_US");
...
EXCEL_METHOD(Sheet, setColWidth)
{
		...
		if (e < s) {
			php_error_docref(NULL TSRMLS_CC, E_WARNING, "Start cell is greater than end cell");
			RETURN_FALSE;
		} else if (s < 0) {
			php_error_docref(NULL TSRMLS_CC, E_WARNING, "Start cell cannot be less than 0");
			RETURN_FALSE;
		} else if (width < 0) {
			php_error_docref(NULL TSRMLS_CC, E_WARNING, "Width cannot be less than 0");
			RETURN_FALSE;
		}
...
EXCEL_METHOD(Sheet, setRowHeight)
{
		...
		if (row < 0) {
			php_error_docref(NULL TSRMLS_CC, E_WARNING, "Row number cannot be less than 0");
			RETURN_FALSE;
		} else if (height < 0) {
			php_error_docref(NULL TSRMLS_CC, E_WARNING, "Height cannot be less than 0");
			RETURN_FALSE;
		}
